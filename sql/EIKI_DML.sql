...
-- /eiki/admin/member/manage/{pageIdx} 회원수 카운트 쿼리
SELECT COUNT(*) AS COUNT
FROM EIKI_MEMBER;

-- /eiki/admin/member/manage/{pageIdx} 회원 리스트 조회 쿼리
SELECT EM.MEMBER_DEC_IDX       AS MEMBER_DEC_IDX,
...
-- /eiki/admin/member/manage/auth/{memberIdx} 회원 권한 토글 쿼리
UPDATE EIKI_MEMBER AS EM
SET EM.IS_ADMIN = IF(EM.IS_ADMIN = 1, 0, 1)
WHERE EM.MEMBER_DEC_IDX = :MEMBER_DEC_IDX;

-- 입력된 날짜 기록이 있는지 확인하는 쿼리
SELECT IFNULL(EAH.ACCESS_DATE, "")   AS ACCESS_DATE,
       IF(COUNT(*) = 1, true, false) AS IS_TODAY_EXIST
FROM EIKI_ACCESS_HISTORY AS EAH
WHERE EAH.ACCESS_DATE = DATE_FORMAT(NOW(), "%Y-%m-%d");

-- 입력된 날짜 기록이 없을 경우 새로운 날짜 기록을 생성하는 쿼리
INSERT INTO EIKI_ACCESS_HISTORY
VALUES (DATE_FORMAT(NOW(), "%Y-%m-%d"), 1);

-- 입력된 날짜 기록이 있을 경우 날짜 기록에 접근 카운트를 증가시키는 쿼리
UPDATE EIKI_ACCESS_HISTORY AS EAH
SET EAH.ACCESS_COUNT = EAH.ACCESS_COUNT + 1
WHERE EAH.ACCESS_DATE = :ACCESS_DATE;

-- 접근 기록을 조회하는 쿼리
SELECT EAH.ACCESS_DATE  AS ACCESS_DATE,
